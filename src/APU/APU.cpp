...
namespace sn {
    ...
    void APU::step() {
        ...
        for (int clocks = sampling_timer.clock(cpu_clock_period_ns); clocks > 0; --clocks) {
            float frame = (
                static_cast<float>(pulse1.sample()) / max_volume_f +
                static_cast<float>(pulse2.sample()) / max_volume_f
            ) / 2.0;
    ...
    Byte APU::readStatus() {
        return (
            pulse1.length_counter.is_enabled() << 0 |
            pulse2.length_counter.is_enabled() << 1
       );
    }
    ...
    FrameCounter APU::setup_frame_counter(IRQ& irq) {
        return FrameCounter(
            {
                std::ref(pulse1.volume), std::ref(pulse1.sweep), std::ref(pulse1.length_counter),
                std::ref(pulse2.volume), std::ref(pulse2.sweep), std::ref(pulse2.length_counter),
            },
            irq
        );
    }
