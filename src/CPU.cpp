...
bool CPU::executeImplied(Byte opcode) {
  switch (static_cast<OperationImplied>(opcode)) {
    ...
    case JMPI: {
      ...
      // 6502 has a bug such that the when the vector of anindirect address begins at the last byte
      // of a page, the second byte is fetched from the beginning of that page rather than the
